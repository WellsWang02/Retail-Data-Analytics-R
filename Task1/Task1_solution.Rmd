...
filePath <- "~/Desktop/Forage/Quantium-Data_Analytics/Task1/"
...
transactions_by_day <- merge(all_dates, transactionData[, .N, by = DATE], all.x=T)
# transactions_by_day <- merge(all_dates, transactionData, by = 'DATE', all.x=T) %>%
#   group_by(DATE) %>%
#   tally()
# names(transactions_by_day) <- c('DATE', 'N')
...
ggplot(transactions_by_day, aes(x = DATE, y = N)) +
geom_line(color = 'cornflowerblue', binwidth=1) +
...
transactionData[, BRAND:= unlist(purrr::map(strsplit(toupper(transactionData$PROD_NAME), ' '),1))]
```
...
* We could also ask our data team for more information. Examples are:
  ...
  + The customer's total spend over the period and total spend for each transaction to understand what proportion of their grocery spend is on chip
...
n_cust <- data[, .(N_CUST = uniqueN(LYLTY_CARD_NBR)), 
               .(LIFESTAGE, PREMIUM_CUSTOMER)][order(-N_CUST)]
#### create plot
p <- ggplot(data = n_cust) +
  geom_mosaic(aes(weight = N_CUST, 
...
avg_units <- data[, .(AVG_UNITS = sum(PROD_QTY)/uniqueN(LYLTY_CARD_NBR)),
                  .(LIFESTAGE, PREMIUM_CUSTOMER)][order(-AVG_UNITS)]
ggplot(data = avg_units, 
            aes(weight = AVG_UNITS, x = LIFESTAGE, fill = PREMIUM_CUSTOMER)) +
...
unit_price <- data[, .(UNIT_PRICE = sum(TOT_SALES)/sum(PROD_QTY)),
                   .(LIFESTAGE, PREMIUM_CUSTOMER)][order(-UNIT_PRICE)]
ggplot(data = unit_price, 
       aes(weight = UNIT_PRICE, x = LIFESTAGE, fill = PREMIUM_CUSTOMER)) +
  geom_bar(position = position_dodge()) +
  labs(x = 'Lifestage', y = 'Avg price per units', 
...
t.test(data[PREMIUM_CUSTOMER=='Mainstream' & 
                  LIFESTAGE %in% c('YOUNG SINGLES/COUPLES', 'MIDAGE SINGLES/COUPLES'), UNIT_PRICE], 
       data[PREMIUM_CUSTOMER!='Mainstream' & 
                  LIFESTAGE %in% c('YOUNG SINGLES/COUPLES', 'MIDAGE SINGLES/COUPLES'), UNIT_PRICE],
       alternative = 'greater')
data <- subset(data, select = -UNIT_PRICE)
...
qty_segment1 <- segment1[, sum(PROD_QTY)]
qty_other <- other[, sum(PROD_QTY)]
brand_qty_segment1 <- segment1[, .(targetSegment = sum(PROD_QTY)/qty_segment1),
                              by = BRAND]
brand_qty_other <- other[, .(other = sum(PROD_QTY)/qty_other),
                         by = BRAND]
brand_proportions <- merge(brand_qty_segment1, 
                           brand_qty_other)[, affinityToBrand := targetSegment/other]
...
qty_segment1_by_packSize <- segment1[, .(targetSegment = sum(PROD_QTY)/qty_segment1),
                              by = PACK_SIZE]
qty_other_by_packSize <- other[, .(other = sum(PROD_QTY)/qty_other),
                         by = PACK_SIZE]
packSize_proportions <- merge(qty_segment1_by_packSize, 
                           qty_other_by_packSize)[, affinityToPackSize := targetSegment/other]
packSize_proportions[order(-affinityToPackSize)]
```
...
```{r, include=FALSE}
T_Sales_C <- data[DATE<as.Date('2018-12-25')&DATE>as.Date('2018-12-16'), sum(TOT_SALES)]
T_Sales_O <- data[!(DATE<as.Date('2018-12-25')&DATE>as.Date('2018-12-16')), sum(TOT_SALES)]
ChristmasSales <- data %>%
  filter(DATE<as.Date('2018-12-25')&DATE>as.Date('2018-12-16'))%>%
  group_by(LIFESTAGE, PREMIUM_CUSTOMER) %>%
  summarise(SALES = sum(TOT_SALES),
            PERC = sum(TOT_SALES)*100/T_Sales_C, .groups='drop')
OtherSales <- data %>%
  filter(!(DATE<as.Date('2018-12-25')&DATE>as.Date('2018-12-16')))%>%
  group_by(LIFESTAGE, PREMIUM_CUSTOMER) %>%
  summarise(SALES = sum(TOT_SALES), 
            PERC = sum(TOT_SALES)*100/T_Sales_O, .groups='drop')
ChristmasPromo <- merge(ChristmasSales, OtherSales, 
                        by=c('LIFESTAGE', 'PREMIUM_CUSTOMER')) %>%
  mutate(PercentageDiff = PERC.x-PERC.y)
ggplot(ChristmasSales)+
