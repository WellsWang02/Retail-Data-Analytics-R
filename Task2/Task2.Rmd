...
filePath <- "~/Desktop/Forage/Quantium-Data_Analytics/Task2/"
...
measureOverTime <- data[, .(totSales = sum(TOT_SALES),
                            ...
                            ), by = .(STORE_NBR, YEARMONTH)][order(YEARMONTH,STORE_NBR)]
...
#### Standardise the magnitude distance so that the measure ranges from 0 to 1
  minMaxDist <- calcDistTable[, .(minDist = min(measure), 
                                  maxDist = max(measure)), by = c("Store1", "YEARMONTH")]
...
#### Over to you! Use the function you created to calculate correlations againststore 77 using total sales and number of customers.
...
```{r Create a combined score composed of correlation and magnitude-1}
...
```{r Create a combined score composed of correlation and magnitude-2}
...
```{r Select control stores with highest score}
...
control_store <- as.numeric(score_Control[order(-finalControlScore)][2,'Store2'])
control_store
```
...
# tracemem(measureOverTimeSales) ==tracemem(measureOverTime)
...
p1<- ggplot(pastSales, aes(TransactionMonth, totSales, color = Store_type)) +
geom_line() +
labs(x = "Month of operation", y = "Total sales", 
     title = "    Trial 77 vs Control 233
...
pastCustomers <- measureOverTime %>%
  ...
  mutate(TransactionMonth = as.Date(paste(YEARMONTH %/% 100,
                                          ...
                                          sep = '-')))%>%
  filter(YEARMONTH <201903)
p2<- ggplot(pastCustomers, aes(TransactionMonth, nCustomers, color = Store_type)) +
geom_line() +
...
percentageDiff <- merge(scaledControlSales, measureOverTimeSales[STORE_NBR == trial_store], 
                        by = 'YEARMONTH')[, percentageDiff := abs(totSales.y/controlSales-1)]
```
...
# pastSales <- measureOverTimeSales[, Store_type := ifelse(STORE_NBR==control_store, 'Control',
#                                                          ifelse(STORE_NBR==trial_store,
#                                                                 'Trial', 'Other stores'))
